...
import json
#========
# --- begin FeatureHasher compatibility shim (for newer scikit-learn) ---
from sklearn.feature_extraction import FeatureHasher as _SKFeatureHasher

class FeatureHasher(_SKFeatureHasher):
    # Accepts: a) a single string, b) a list/tuple of strings (one sample),
    # or c) the standard iterable-of-iterables.
    def transform(self, X, *args, **kwargs):
        # single string -> [[string]]
        if isinstance(X, (str, bytes)):
            X = [[X]]
        # list/tuple of strings -> [list_of_strings]
        elif isinstance(X, (list, tuple)) and (len(X) > 0) and isinstance(X[0], (str, bytes)):
            X = [list(X)]
        return super().transform(X, *args, **kwargs)
# --- end FeatureHasher compatibility shim ---
...
class PEFeatureExtractor(object):
    ...
    def raw_features(self, bytez):
        ...
        if isinstance(lief_binary, lief.lief_errors):
            lief_binary = None
            if self.debug:
                print(f"LIEF parsing error: {lief_binary}")
